...
This Erlang module allows you to construct an Erlang digraph from objects in a bucket. Each object will result in a vertex being created, identified by the key, and optionally, the object value will be used for the vertex label. Links on objects will result in edges being created as specified by the links. An optional filter list can be specified which will result in only specific objects matching the filter being used to construct the digraph. If the filter list is omitted, the entire bucket will be used.
...
# digraph_exporter:export_digraph(address of server, port, bucket, digraph, boolean use object value for label). or
# digraph_exporter:export_digraph(address of server, port, bucket, filter_list, digraph, boolean use object value for label).
